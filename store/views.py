...
class ProductViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if OrderItem.objects.filter(product_id=kwargs['pk']).count() > 0:
            return Response({'error': 'product can not be deleted because it associated with an order item'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
...
class CollectionViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if Product.objects.filter(collection__id=kwargs['pk']).count() > 0:
            return Response({'error': 'collection can not be deleted because it is associated with one or more product'})
        return super().destroy(request, *args, **kwargs)
